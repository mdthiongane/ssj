 ...
 */
public class TallyHistogram_old extends Tally {
    ...
    */
   public TallyHistogram_old(double a, double b, int s) {
      super();
      init (a, b, s);
   }
    ...
    *  @param s            number of bins
    */
   public TallyHistogram_old (String name, double a, double b, int s) {
      super (name);
      init (a, b, s);
   }
    ...
    */
   public TallyHistogram_old clone() {
      TallyHistogram_old image = (TallyHistogram_old)super.clone();
   ...
   /**
    * Remove empty bins in the tails (left and right), 
    * without changing the bin size.
    */
   public TallyHistogram_old  trimHistogram (){
	TallyHistogram_old image = (TallyHistogram_old)super.clone(); 
	 ...
	 System.arraycopy (co, i, coco, 1, j-i+1);
	 System.arraycopy (co, 0, coco, 0, 1);
	 System.arraycopy (co, co.length-1, coco, coco.length-1, 1);
	 image.co = coco;
	 image.m_h = m_h;
   ...
   /**
    * Merges this histogram with the other histogram, 
    * by adding the bin counts of the two histograms.
    *   @param other    the histogram to add
    * 
    * Returns the merged histogram.
    */
   public TallyHistogram_old  addHistogram (TallyHistogram_old other){
   ...
   /**
   * Merges bins by groups of size $g$. If there are $m$ bins initially, 
   *the new number of bins will be $\lceil m/g\rceil$.
   *The last bin may regroup less than $g$ original bins if $m$ is not a multiple of $g$
   **/
   public TallyHistogram_old  aggregateBins (int g){
	   TallyHistogram_old image = (TallyHistogram_old)super.clone();
	   double numB=Math.ceil((double)numBins/(double)g);
	   int  numBins2=(int)numB;
	   int[] coco = new int[2 +numBins];
	   System.arraycopy (co, 0, coco, 0, 2 + numBins);
	   int[] coco2 = new int[2 +numBins2]; 
	   int b=1;
	   for(int j=1;j<numBins2;j++)
	   { for(int i=b;i<b+g;i++)
			coco2[j]=coco2[j]+coco[i];
		  b=b+g;
	    } 
	    while(b<coco.length-1)
	    {	 coco2[numBins2]=coco2[numBins2]+coco[b];
	         b++;
	    }
	    coco2[0]=coco[0];
	    coco2[coco2.length-1]=coco[coco.length-1];
	    image.co = coco2;
	    image.m_h = m_h*g;
	    image.m_a = m_a;
	    image.m_b = m_h*numBins2;
	    image.numBins =numBins2;
	   return image;
   }
